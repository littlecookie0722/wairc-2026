...
- [ ] `python -m pytest` passes locally with 201 tests.
...
- [ ] `wairc --version`, `wairc --help`, both doctor output modes, both
  ...
  pass locally.
- [ ] Source and wheel distributions build successfully, include the v0.16.0
  release documentation, and include both benchmark fixtures in the source
  distribution.
- [ ] The wheel installs in an isolated virtual environment with runtime
...
- [ ] `pyproject.toml`, `src/__init__.py`, and `CITATION.cff` use `0.16.0`.
...
- [ ] README and `ROADMAP.md` release text and links are prepared for
  `v0.16.0` publication.
- [ ] Annotated tag `v0.16.0` points to the verified release commit.
- [ ] GitHub release `v0.16.0` is published with verified notes, wheel/sdist
